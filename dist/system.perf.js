  ...
  return function() {
    ...
    */
    ...
    function define(name, deps, factory) {
      ...
      // console.warn('name', name, 'deps', deps, 'factory', factory);
...
(function() {
  ...
  // Modif vodalys
  var amdRegEx = /(?:^\uFEFF?|[^$_a-zA-Z\xA0-\uFFFF.])define\s*\(\s*("[^"]+"\s*,\s*|'[^']+'\s*,\s*)?\s*(\[(:?\s*.*,\s*)*\s*.*\s*\]|function\s*|{|[_$a-zA-Z\xA0-\uFFFF][_$a-zA-Z0-9\xA0-\uFFFF]*\))/gm;

  hook('instantiate', function(instantiate) {
    return function(load) {
      var loader = this;

      // console.warn(load, loader, 'plop');
